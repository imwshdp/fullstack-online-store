...
import { createOrder, fetchOrders } from 'store/slices/orders/actions';
	...
	return (
		<section className={css.BasketListWrapper}>
			...
			{productsViewList.map(product => (
				<div className={css.BasketListRow} key={product.name}>
					{product.name}

					<Counter increase={() => increaseQuantity(product.id)} decrease={() => decreaseQuantity(product.id)}>
						{product.quantity}
					</Counter>
				</div>
			))}
			...
			{!basketState.products.length ? (
				...
				<Button
					...
					color={'var(--lightgray)'}>
					{basketState.loading ? <ButtonLoader /> : window.innerWidth > 1000 ? 'Оформить заказ' : <EditIcon />}
				</Button>
			)}
		</section>
	);
